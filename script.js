...
document.addEventListener("keydown", (e) => {
    ...
    let col = Math.round((x - 4)/m * 2 + 1);
    let row = Math.round((y - 2)/m * 2 + 1);
    
    if(col < 0) col = 0;
    if(col >= arr.length) col = arr.length - 1;
    if(row < 0) row = 0;
    if(row >= arr[0].length) row = arr[0].length - 1;
    
    switch(key){
        case "w":
        case "W":
        case "ArrowUp":
            if(y > 2 && arr[col][row - 1] === 0){
                ctx2.clearRect(0,0, canvas.width, canvas.height);
                y -= m;
